...
/**
 * GET /api/mattermost/review-queue/settings
...
export async function GET() {
    try {
        ...
        return NextResponse.json({
            success: true,
            data: { dismissedAutoCleanupDays: general?.mattermost?.dismissedAutoCleanupDays || 0 },
        });
    } catch (error) {
...
/**
 * PATCH /api/mattermost/review-queue/settings
...
export async function PATCH(request: NextRequest) {
    try {
        ...
        const mattermost = general.mattermost || {};
        mattermost.dismissedAutoCleanupDays = parsed.data.dismissedAutoCleanupDays;

        await prisma.companySettings.upsert({
            where: { companyId },
            update: { generalSettings: { ...general, mattermost } },
            create: { companyId, generalSettings: { mattermost } },
        });

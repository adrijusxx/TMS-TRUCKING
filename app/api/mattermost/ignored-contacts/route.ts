...
/**
 * GET /api/mattermost/ignored-contacts
 * List permanently ignored Mattermost contacts for the company
 */
export async function GET(request: NextRequest) {
    try {
        ...
        const where: any = { companyId, platform: 'MATTERMOST' };
        if (search) {
            where.OR = [
        ...
        const [items, total] = await Promise.all([
            prisma.messagingIgnoredContact.findMany({
                where,
                include: {
            ...
            prisma.messagingIgnoredContact.count({ where }),
        ]);

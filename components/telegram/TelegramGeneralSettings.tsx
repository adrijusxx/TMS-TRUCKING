...
interface Props {
    ...
    onUpdate: (data: Partial<TelegramSettings>) => void;
}

export default function TelegramGeneralSettings({ settings, onToggle, onUpdate }: Props) {
    return (
        <Card>
            ...
            <CardContent className="space-y-4">
                ...
                <ToggleRow
                    ...
                    description="Let AI automatically respond to driver messages"
                ...
                {settings.businessHoursOnly && (
                    <div className="grid grid-cols-3 gap-3 ml-6">
                        ...
                        <div className="space-y-1">
                            ...
                            <Select value={settings.timezone || 'America/Chicago'} onValueChange={v => onUpdate({ timezone: v })}>
                                <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                                <SelectContent>
                                    {TIMEZONES.map(tz => (
                                        <SelectItem key={tz} value={tz} className="text-xs">{tz.replace('America/', '').replace('Pacific/', '').replace(/_/g, ' ')}</SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                        </div>
                ...
                <div className="space-y-1.5">
                    <Label className="text-xs">Admin Notification Chat ID</Label>
                    <Input
                        value={settings.adminChatId || ''}
                        onChange={e => onUpdate({ adminChatId: e.target.value || null })}
                        placeholder="Telegram Chat ID for admin notifications"
                        className="h-8 text-xs"
                    />
                    <p className="text-[10px] text-muted-foreground">
                        Optional. Receives notifications for critical events.
                    </p>
                </div>
            </CardContent>
        </Card>

...
import { Truck, ExternalLink, MapPin, ChevronLeft, ChevronRight, Wrench } from 'lucide-react';
import Link from 'next/link';
import SortableColumnHeader from './SortableColumnHeader';
import InventoryTableToolbar from './InventoryTableToolbar';
import MarkOOSDialog from './MarkOOSDialog';
import type { TruckInventoryItem, InventoryResponse, OOSEquipmentRef } from '@/lib/managers/fleet-monitoring/types';
...
export default function TruckInventoryTable() {
  ...
  const { data, isLoading, refetch } = useQuery<{ success: boolean; data: InventoryResponse<TruckInventoryItem> }>({
    queryKey: ['truck-inventory', page, sort, status, search],
    queryFn: async () => {
      const params = new URLSearchParams({
        type: 'truck', page: String(page), limit: String(limit),
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2 text-base">
              <Truck className="h-4 w-4" /> Trucks
            </CardTitle>
          </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b text-left text-muted-foreground">
                      <SortableColumnHeader label="Unit #" column="truckNumber" currentSort={sort} onSort={handleSort} />
                  ...
                  <tbody>
                    {items.map((truck) => {
                      const idle = idleBadge(truck.daysSinceLastLoad);
                      return (
                        <tr key={truck.id} className="border-b last:border-0 hover:bg-muted/30">
                          ...
                          <td className="py-2 pr-3 text-xs">
                            {truck.currentDriver ? (
                              <span>{truck.currentDriver.name}</span>
                            ) : (
                          ...
                          <td className="py-2 pr-3 text-xs">
                            {truck.activeLoad ? (
                              <div>
                                <div className="font-medium">{truck.activeLoad.lane}</div>
                                <div className="text-muted-foreground">
                                  {formatDate(truck.activeLoad.pickupDate)} – {formatDate(truck.activeLoad.deliveryDate)}
                                </div>
                              </div>
                            ) : (
                          ...
                          <td className="py-2 pr-3 text-xs">
                            {truck.lastLoad ? (
                              <div>
                                <span>{truck.lastLoad.loadNumber}</span>
                                <span className="text-muted-foreground ml-1">{formatDate(truck.lastLoad.date)}</span>
                              </div>
                            ) : (
                          ...
                          <td className="py-2 pr-3 text-xs max-w-[140px]">
                            {truck.samsaraLocation ? (
                              <div className="flex items-center gap-1">
                                <MapPin className="h-3 w-3 text-muted-foreground shrink-0" />
                                <span className="truncate">{truck.samsaraLocation.address}</span>
                              </div>
                            ) : (
                          ...
                          <td className="py-2 pr-3 text-xs">
                            {truck.oosInfo.longTermOutOfService ? (
                              <div className="flex items-center gap-1 text-red-600">
                                ...
                                <span className="truncate max-w-[80px]">{truck.oosInfo.reason || 'OOS'}</span>
                              </div>
                            ) : null}
                          </td>
                          <td className="py-2">
                            <div className="flex items-center gap-1">
                              {!truck.oosInfo.longTermOutOfService && (
                                <Button variant="ghost" size="sm" className="text-xs h-7"
                                  onClick={() => setOosTarget({ id: truck.id, number: truck.truckNumber, type: 'TRUCK' })}>
                                  Mark OOS
                                </Button>
                              )}

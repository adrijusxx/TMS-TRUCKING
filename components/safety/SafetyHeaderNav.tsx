...
import {
  safetyStandaloneItems,
  safetyNavigationSections,
} from '@/lib/safety-navigation-config';
...
export default function SafetyHeaderNav() {
  const pathname = usePathname();

  // Check if any item in a section is active
  const isSectionActive = (section: typeof safetyNavigationSections[0]) => {
    return section.items.some(
      (item) => pathname === item.href || pathname?.startsWith(item.href + '/')
    );
  };

  // Check if standalone item is active
  const isStandaloneItemActive = (href: string) => {
  ...
  return (
    <div className="flex items-center gap-1 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" className="w-64 max-h-[calc(100vh-8rem)] overflow-y-auto">
            ...
            {safetyStandaloneItems.map((item) => {
              ...
              const isActive = isStandaloneItemActive(item.href);
              return (
            ...
            {safetyNavigationSections.map((section) => {
              ...
              return (
                <div key={section.title}>
                  ...
                  {section !== safetyNavigationSections[safetyNavigationSections.length - 1] && (
                    <DropdownMenuSeparator />
                  )}
                </div>
              );
      ...
      {/* Desktop: Horizontal tabs + dropdowns */}
      <div className="hidden lg:flex items-center gap-1">
        {/* Standalone items as tabs */}
        {safetyStandaloneItems.map((item) => {
          const ItemIcon = item.icon;
          const isActive = isStandaloneItemActive(item.href);
          return (
